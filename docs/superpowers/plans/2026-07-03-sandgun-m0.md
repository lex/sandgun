...
### Task 10: Liquid rest state (amendment — user decision 2026-07-03)

> Task 9's acceptance exposed that liquids never rest: pool surfaces slosh 1 cell forever,
> keeping ~half the generated world's chunks permanently active and defeating chunk sleeping.
> User chose **rest-seeking dispersion**: liquids slide sideways only toward an actual drop.

**Files:**
- Modify: `crates/sandgun-core/src/world.rs` (horizontal-dispersion section of `update_liquid` only)
- Modify: `crates/sandgun-core/tests/sim.rs` (replace `water_falls_then_spreads_sideways`; add rest tests)
- Modify: `crates/sandgun-core/tests/worldgen.rs` (add full-world settling test)

**Interfaces:**
- Consumes: existing `update_liquid` (fall/diagonal/density-swap sections unchanged), `swap_cells`, `DISPERSION`.
- Produces: the new rule — while scanning up to `DISPERSION` cells sideways (blocked by any
  non-empty cell), move to the FIRST empty cell whose below-neighbor is Empty or a strictly
  lighter liquid ("a drop"); if neither direction offers a drop, DO NOT MOVE. The world floor
  and solids below are not drops. Resting cells make no swap, so their chunks sleep naturally.
- Known accepted quirks (documented, not bugs): water poured onto flat ground heaps at ~45°
  instead of spreading thin; pool surfaces can stay rough by ±1 cell; oil dropped onto a water
  pool rests as a lump instead of slicking outward. All are M1 flow-model polish items.

- [ ] **Step 1: Update the tests**

In `crates/sandgun-core/tests/sim.rs`, DELETE `water_falls_then_spreads_sideways` and add:
```rust
#[test]
...
In `crates/sandgun-core/tests/worldgen.rs`, add:
```rust
#[test]
...
- [ ] **Step 2: Run tests — the three new/changed ones must FAIL against current code**

Run: `cargo test -p sandgun-core`
Expected: `pooled_water_comes_to_rest` and `generated_world_fully_settles` FAIL (cells_processed
never reaches 0); the ledge test may pass already — note which failed in the report.

- [ ] **Step 3: Implement rest-seeking dispersion**

In `crates/sandgun-core/src/world.rs`, replace ONLY the horizontal-dispersion section of
`update_liquid` (from the `// Horizontal dispersion:` comment to the end of the function) with:
```rust
...
Run: `cargo test -p sandgun-core`
Expected: all 20 pass (previous 18, minus the deleted spread test, plus 3 new).
`liquids_do_not_pass_through_rock` must still pass — water resting in the cup satisfies it.

- [ ] **Step 5: Rebuild wasm + browser-verify settling**

```bash
wasm-pack build crates/sandgun-wasm --release --target web --out-dir ../../web/src/pkg
```
Then with the dev server: generate a world, wait ~15s idle, toggle `D` — expected: red chunk
boxes all disappear and the cells counter reads 0. Press `N` a few times to confirm fresh worlds
also settle. Painting water onto a slope must still flow downhill to a pool.

- [ ] **Step 6: Commit**

```bash
git add -A
git commit -m "fix: rest-seeking liquid dispersion so worlds settle (M0 task 10)"
```
