...
### Task 8: Mouse painting + material/brush controls

**Files:**
- Create: `web/src/input.js`
- Modify: `web/src/main.js` (wire input; apply before `world.step()`)

**Interfaces:**
- Consumes: `world.paint`, `M` material map from Task 7.
- Produces: `attachInput(canvas, worldW, worldH) -> input` and `applyInput(input, world)` (call once per frame). Controls — left-drag: paint; keys `1..4`: sand/water/oil/rock; `0` or `E`: eraser; `[` / `]`: brush radius 1–24 (default 4). `input.status` string ("sand r4") for the overlay.

- [ ] **Step 1: Write input handling**
...
export function attachInput(canvas, worldW, worldH) {
  const input = {
    ...
    material: M.SAND, radius: 4, debug: false,
...
In `web/src/main.js`, add the import and wire it:
```js
...
Reload the page. Expected: left-drag rains sand; `2` + drag pours water that flows and layers under oil; `4` draws rock walls that block liquids; `0` erases; `[`/`]` change brush size visibly; fast drags paint continuous strokes, not dots.

- [ ] **Step 3: Commit**

```bash
git add -A
git commit -m "feat: mouse painting with material and brush controls (M0 task 8)"
```
